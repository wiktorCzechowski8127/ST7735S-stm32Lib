...
static SPI_HandleTypeDef* lcdSpiData;
...
static void lcd_cmd(uint8_t cmd)
{
	...
	HAL_SPI_Transmit(lcdSpiData, &cmd, 1, HAL_MAX_DELAY);
...
static void lcd_data(uint8_t data)
{
	...
	HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit(lcdSpiData, &data, 1, HAL_MAX_DELAY);
	HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_SET);
}
...
void lcd_print(void)
{
	...
	HAL_GPIO_WritePin(LCD_DC_GPIO_Port, LCD_DC_Pin, GPIO_PIN_SET);
	HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit_DMA(lcdSpiData, (uint8_t*)frame_buffer, sizeof(frame_buffer));
}
...
{
	if (HAL_SPI_GetState(lcdSpiData) == HAL_SPI_STATE_BUSY)
		return 1;
	else
		return 0;
}
